...
def download_data(save_dir, list_path, isListed, start_y, start_m):
    ...
    for idx, ticker in enumerate(tickers):
        ...
        try:
            if isListed:
                stock = Stock(ticker[:-3], initial_fetch=False)
            else:
                stock = Stock(ticker[:-4], initial_fetch=False)
            
            data = stock.fetch_from(start_y, start_m)
            ...
            if not data.empty:
                ...
                #rename the columns and drop some useless columns
                data.rename(columns={'capacity':'Volume',
                                     'open':'Open',
                                     'close':'Close',
                                     'high':'High',
                                     'low':'Low',
...
def main():
    ...
    download_data(save_dir,TWSE_path,True,2023,1)

    #OTC
    OTC_path = '~/Documents/Dev/Cheater_finder/Stock_project/TW_stock_data/OTCs.csv'
    download_data(save_dir,OTC_path,False,2023,1)
